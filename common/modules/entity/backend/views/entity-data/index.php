...
$columns[] = [
    ...
    'buttons'=>[
        'view'=>function ($url, $model) {
            $customUrl = Yii::$app->getUrlManager()->createUrl([Config::BACKEND_MODULE_NAME.'/entity-data/view','id'=>Yii::$app->request->get('id'), 'item_id' => $model['id']]);
            return Html::a( '<span class="glyphicon glyphicon-eye-open"></span>', $customUrl, ['title' => Yii::t('yii', 'View'), 'data-pjax' => 0]);
        },
        'update'=>function ($url, $model) {
            $customUrl = Yii::$app->getUrlManager()->createUrl([Config::BACKEND_MODULE_NAME.'/entity-data/update','id'=>Yii::$app->request->get('id'), 'item_id' => $model['id']]);
            return Html::a( '<span class="glyphicon glyphicon-pencil"></span>', $customUrl, ['title' => Yii::t('yii', 'Update'), 'data-pjax' => 0]);
        },
        'delete'=>function ($url, $model) {
            $customUrl = Yii::$app->getUrlManager()->createUrl([Config::BACKEND_MODULE_NAME.'/entity-data/delete','id'=>Yii::$app->request->get('id'), 'item_id' => $model['id']]);
